...
function App() {
  ...
  const handleSubmit = async (newClientData) => {
    if (modalMode === 'add') {
      try {
        const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/clients`, newClientData); // Replace with your actual API URL
      ...
      console.log('Updating client with ID:', clientData.id); // Log the ID being updated
            try {
                const response = await axios.put(`${import.meta.env.VITE_BACKEND_URL}/api/clients/${clientData.id}`, newClientData);
